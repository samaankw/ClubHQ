...
describe("findViolations", () => {
  ...
  it("does not let a URL's // swallow a real violation on the same line", () => {
    const v = findViolations('const s = { uri: "https://cdn.example.com/a.png", borderRadius: 8 };', "app/x.tsx");
    expect(v).toHaveLength(1);
...
describe("lintScoped", () => {
  ...
  it("PENDING does not list the screens already converted by the design-system plan", () => {
    expect(PENDING).toEqual(expect.not.arrayContaining(["app/(tabs)/dashboard.tsx", "app/(tabs)/profile.tsx", "app/(tabs)/players.tsx"]));
  });
});
