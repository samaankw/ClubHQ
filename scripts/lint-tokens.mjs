...
export function evaluate({ files, pending }) {
  ...
  const unexpected = Object.keys(files).filter((f) => !pendingSet.has(f) && files[f] > 0);
...
function main() {
  ...
  if (unexpected.length) {
    failed = true;
    const violations = unexpected.flatMap((f) => findViolations(fs.readFileSync(path.join(root, f), "utf8"), f));
  ...
  console.log(`Token lint passed — ${scanned.length} file(s) scanned, ${PENDING.length} still pending conversion.`);
}
