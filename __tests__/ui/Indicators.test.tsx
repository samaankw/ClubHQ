...
describe("ProgressBar", () => {
  ...
  it("clamps out-of-range values", async () => {
    expect(
      flat((await render(<ProgressBar value={2} />)).getByTestId("progress-fill").props.style).width
    ).toBe("100%");
    expect(
      flat((await render(<ProgressBar value={-1} />)).getByTestId("progress-fill").props.style).width
    ).toBe("0%");
  });
