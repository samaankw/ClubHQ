  ...
  const renderHomeworkRow = (h: HomeworkItem & { drill?: Drill | null }) => (
    ...
    >
      ...
      <View style={{ flex: 1, gap: space[1] }}>
        ...
        {h.description ? <Text tone="secondary" role="bodySm">{h.description}</Text> : null}
        {h.drill?.video_url ? (
          <Button
    ...
    <>
      <Screen>
        ...
        <SpotlightCard style={{ flexDirection: "row", alignItems: "center", gap: space[3] }}>
          ...
          <View style={{ flex: 1, gap: space[1] }}>
            <Text role="h1" tone="onSpotlight">{player.full_name}</Text>
        ...
        {plan ? (
          <>
            ...
            {needsStaffReview ? (
              <AICard style={{ gap: space[3] }}>
                ...
                <Text role="h2" tone="onSpotlight">Coach review required</Text>
                ...
                <View style={{ flexDirection: "row", gap: space[2] }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Button label={publishing ? "Saving…" : "Publish to Parent"} fullWidth onPress={() => reviewPlan(true)} disabled={publishing} />
                  </View>
                </View>
              </AICard>
            ) : showParentHeadline ? (
            ...
            <Card style={{ gap: space[3] }}>
              ...
              {(plan.priorities ?? []).map((p, idx) => (
                <View key={`${p.skill}-${idx}`} style={{ flexDirection: "row", gap: space[3], alignItems: "flex-start" }}>
                  ...
                  <View style={{ flex: 1, gap: space[1] }}>
                    <Text role="h3" style={{ textTransform: "capitalize" }}>{p.skill.replace(/_/g, " ")}</Text>
                    <Text tone="secondary" role="bodySm">{p.note}</Text>
                  </View>
                </View>
              ))}
            </Card>
            ...
            <Card style={{ gap: space[3] }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <Text role="label" tone="brand">{completionPct}% done</Text>
              </View>
              ...
              {isStaff && (
                <Text role="caption" tone="tertiary">Homework completion is parent-controlled in this build.</Text>
              )}
            </Card>
                    ...
                    <View key={pp.id} style={{ gap: space[2] }}>
                      ...
                      >
                        ...
                        <View style={{ flexDirection: "row", alignItems: "center", gap: space[1] }}>
                          <Text role="label" tone="secondary">{pastPct}% done</Text>
          ...
          <Card>
            <Text tone="secondary">
              {isStaff ? `No development plan yet. Evaluate ${player.full_name} to create a coach-reviewable AI draft.` : "No published development plan yet."}
            </Text>
          </Card>
        )}

        {evaluations.length === 0 && !plan && (
          <EmptyState icon="stats-chart" title="No evaluations yet" body="Once a coach logs an evaluation, this player's development profile appears here." />
        )}
        ...
        {(profile?.role === "director" || isLinkedParent) && (
          <Card style={{ gap: space[2] }}>
            ...
            <Text tone="secondary">
              Permanently delete {player.full_name}'s player record, evaluations, development plans, homework, and linked history. This can't be undone.
            </Text>
...
function StatTileValue({ label, value }: { label: string; value: number | null }) {
  return (
    <Card style={{ gap: space[1] }}>
      ...
      <Text role="display" tone="brand">{value ?? "—"}</Text>
    </Card>
  );
}
