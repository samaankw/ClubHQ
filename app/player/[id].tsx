  ...
  const renderHomeworkRow = (h: HomeworkWithDrill) => (
    <Pressable key={h.id} disabled={!isLinkedParent} onPress={() => toggleHomework(h)} style={styles.hwRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {h.drill?.video_url ? (
          <Pressable onPress={() => setWatchingDrill(h)} style={styles.watchButton}>
            <Text style={styles.watchButtonText}>▶ Watch drill video</Text>
          </Pressable>
        ) : null}
      </View>
    </Pressable>
      ...
      <ScrollView style={styles.container}>
            ...
            {trend.map((evaluation, index) => (
              <View key={evaluation.id} style={styles.historyRow}>
                ...
                <View style={styles.historyTrack}>
                  <View style={[styles.historyFill, { width: `${Math.max(0, Math.min(100, (evaluation.score ?? 0) * 10))}%` }]} />
                </View>
                <Text style={styles.historyScore}>{evaluation.score ?? "—"}</Text>
                {index > 0 && trend[index - 1].score != null && evaluation.score != null ? (
                  <Text style={styles.historyDelta}>
                    {evaluation.score! - trend[index - 1].score! > 0 ? "↑" : evaluation.score! - trend[index - 1].score! < 0 ? "↓" : "→"}
                  </Text>
                ) : (
                  <Text style={styles.historyDelta}>·</Text>
                )}
              </View>
            ))}
          ...
          <>
            ...
            {isStaff && plan.status !== "published" && (
              <View style={styles.reviewCard}>
                ...
                <Text style={styles.reviewBody}>
                  AI created this as a draft. Check the priorities and homework before releasing it to the parent.
                </Text>
                <View style={styles.reviewActions}>
                  <Pressable style={styles.secondaryButton} onPress={() => reviewPlan(false)} disabled={publishing}>
                    <Text style={styles.secondaryText}>Mark Reviewed</Text>
                  </Pressable>
                  <Pressable style={styles.publishButton} onPress={() => reviewPlan(true)} disabled={publishing}>
                    <Text style={styles.publishText}>{publishing ? "Saving…" : "Review & Publish"}</Text>
                  </Pressable>
                </View>
              </View>
            )}
            ...
            <View style={styles.card}>
              ...
              {(plan.priorities ?? []).map((p, idx) => (
                <View key={`${p.skill}-${idx}`} style={styles.priorityRow}>
                  <Text style={styles.priorityNum}>{idx + 1}</Text>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.prioritySkill}>{p.skill.replace(/_/g, " ")}</Text>
                    <Text style={styles.priorityNote}>{p.note}</Text>
                  </View>
                </View>
              ))}
            </View>
            ...
            <View style={styles.card}>
              <View style={styles.homeworkHeader}>
                <Text style={styles.cardLabel}>THIS WEEK'S HOMEWORK</Text>
                <Text style={styles.pct}>{completionPct}% done</Text>
              </View>
              {homework.length === 0 ? (
                  ...
                  return (
                    <View key={pp.id} style={styles.pastWeekBlock}>
                      <Pressable onPress={() => togglePastPlan(pp.id)} style={styles.pastWeekHeader}>
                        ...
                        <Text style={styles.pastWeekPct}>
                          {pastPct}% done {isExpanded ? "▲" : "▼"}
                        </Text>
                      </Pressable>
                      {isExpanded &&
        ...
        {(profile?.role === "director" || isLinkedParent) && (
          <View style={styles.dangerCard}>
            ...
            <Pressable style={styles.dangerButton} onPress={confirmDeletePlayer}>
              <Text style={styles.dangerButtonText}>Delete {player.full_name}'s Data</Text>
            </Pressable>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  centered: { flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#0B0B0D" },
  identityCard: { marginBottom: 14 },
  playerName: { fontSize: 24, fontWeight: "800", color: "#F2F2F3" },
  playerMeta: { color: "#9A9DA3", marginTop: 3 },
  scoreCard: { backgroundColor: "#0A6CFF", borderRadius: 14, padding: 20, marginBottom: 14 },
  ...
  scoreLabel: { fontSize: 12, fontWeight: "700", color: "#CFE0F0", letterSpacing: 0.5, marginBottom: 6 },
  scoreBig: { fontSize: 30, fontWeight: "800", color: "#fff" },
  dated: { color: "#CFE0F0", marginTop: 6, fontSize: 12 },
  ...
  body: { fontSize: 15, color: "#B5B8BE", lineHeight: 21 },
  reviewCard: { backgroundColor: "#2A2410", borderRadius: 14, padding: 16, marginBottom: 14 },
  reviewTitle: { fontSize: 15, fontWeight: "800", color: "#F5D061" },
  reviewBody: { fontSize: 13, lineHeight: 18, color: "#D4C486", marginTop: 5 },
  reviewActions: { flexDirection: "row", gap: 8, marginTop: 12 },
  secondaryButton: { flex: 1, borderWidth: 1, borderColor: "#0A6CFF", borderRadius: 9, padding: 10, alignItems: "center" },
  secondaryText: { color: "#0A6CFF", fontWeight: "700", fontSize: 12 },
  publishButton: { flex: 1, backgroundColor: "#0A6CFF", borderRadius: 9, padding: 10, alignItems: "center" },
  publishText: { color: "#fff", fontWeight: "700", fontSize: 12 },
  priorityRow: { flexDirection: "row", gap: 10, marginBottom: 10, alignItems: "flex-start" },
  priorityNum: {
    width: 22,
    height: 22,
    borderRadius: 11,
  ...
  prioritySkill: { fontSize: 15, fontWeight: "700", color: "#F2F2F3", textTransform: "capitalize" },
  priorityNote: { fontSize: 13, color: "#9A9DA3", marginTop: 2 },
  homeworkHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
  pct: { fontSize: 13, fontWeight: "700", color: "#0A6CFF" },
  hwRow: { flexDirection: "row", gap: 12, alignItems: "flex-start", paddingVertical: 8, borderTopWidth: 1, borderTopColor: "#242424" },
  checkbox: {
    width: 22,
    height: 22,
    borderRadius: 6,
  ...
  checkboxDone: { backgroundColor: "#0A6CFF" },
  checkmark: { color: "#fff", fontSize: 13, fontWeight: "800" },
  hwDay: { fontSize: 11, fontWeight: "700", color: "#0A6CFF" },
  hwTitle: { fontSize: 15, fontWeight: "700", color: "#F2F2F3" },
  hwTitleDone: { textDecorationLine: "line-through", color: "#6B6F76" },
  hwDesc: { fontSize: 13, color: "#9A9DA3", marginTop: 2 },
  watchButton: {
    marginTop: 8,
  ...
  historyRow: { flexDirection: "row", alignItems: "center", gap: 8, marginVertical: 6 },
  historyDate: { width: 43, fontSize: 11, color: "#9A9DA3" },
  historyTrack: { flex: 1, height: 8, backgroundColor: "#1C1D20", borderRadius: 4, overflow: "hidden" },
  historyFill: { height: 8, backgroundColor: "#0A6CFF", borderRadius: 4 },
  historyScore: { width: 28, textAlign: "right", fontWeight: "800", color: "#0A6CFF", fontSize: 12 },
  historyDelta: { width: 14, color: "#30D158" },
  historyHint: { color: "#6B6F76", fontSize: 10, lineHeight: 14, marginTop: 8 },
  dangerCard: { backgroundColor: "#2A1614", borderRadius: 14, padding: 16, marginTop: 8, marginBottom: 30 },
  dangerLabel: { fontSize: 11, fontWeight: "700", color: "#FF6B6B", letterSpacing: 0.5, marginBottom: 8 },
  dangerBody: { fontSize: 13, color: "#E0A199", lineHeight: 18, marginBottom: 12 },
  dangerButton: { backgroundColor: "#FF453A", borderRadius: 10, padding: 12, alignItems: "center" },
