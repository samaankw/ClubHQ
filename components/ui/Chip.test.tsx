...
import { Chip, FilterChipRow } from "./Chip";
...
describe("FilterChipRow", () => {
  it("reports the chosen option", async () => {
    const fn = jest.fn();
    const { getByLabelText } = await render(<FilterChipRow options={["All", "Weather"]} value="All" onChange={fn} />);
...
describe("SegmentedControl", () => {
  it("marks the active segment as selected", async () => {
    const { getByLabelText } = await render(<SegmentedControl options={["Events", "Announcements"]} value="Events" onChange={() => {}} />);
  ...
  it("reports a segment change", async () => {
    const fn = jest.fn();
    const { getByLabelText } = await render(<SegmentedControl options={["Events", "Announcements"]} value="Events" onChange={fn} />);
