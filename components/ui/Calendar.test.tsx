...
describe("Calendar", () => {
  it("renders the correct number of day cells for a known month", async () => {
    const { getAllByTestId } = await render(<Calendar value={AUG_15_2026} onChange={() => {}} minDate={FAR_PAST} />);
    expect(getAllByTestId("calendar-day")).toHaveLength(31);
  });

  it("calls onChange with the pressed date", async () => {
    const onChange = jest.fn();
    const { getByLabelText } = await render(<Calendar value={AUG_15_2026} onChange={onChange} minDate={FAR_PAST} />);
  ...
  it("marks the selected day with accessibilityState.selected", async () => {
    const { getByLabelText } = await render(<Calendar value={AUG_15_2026} onChange={() => {}} minDate={FAR_PAST} />);
  ...
  it("does not call onChange when a day before minDate is pressed", async () => {
    const onChange = jest.fn();
    const { getByLabelText } = await render(<Calendar value={AUG_15_2026} onChange={onChange} minDate={new Date(2026, 7, 10)} />);
    await fireEvent.press(getByLabelText("Wednesday, August 5, 2026"));
  ...
  it("still disables past days when minDate is supplied", async () => {
    const { getByLabelText } = await render(<Calendar value={AUG_15_2026} onChange={() => {}} minDate={new Date(2026, 7, 10)} />);
  ...
  it("moves the visible month with prev/next without emitting onChange", async () => {
    const onChange = jest.fn();
    const { getByText, getByLabelText } = await render(<Calendar value={AUG_15_2026} onChange={onChange} minDate={FAR_PAST} />);
